...
export function CommandPalette({
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowDown') {
        ...
        e.preventDefault();
        setSelectedIndex(
          (prev) => (prev - 1 + filteredBooks.length) % filteredBooks.length,
        );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[15vh]">
      {/* Backdrop */}
      <div className="fixed inset-0 bg-surface " onClick={onClose} />
        ...
        <div className="relative overflow-hidden rounded-2xl border border-border bg-surface  ">
          ...
          <div className="flex items-center border-b border-border px-4">
            ...
            <input
              ...
              className="flex-1 bg-transparent px-4 py-4 text-foreground placeholder-zinc-500 outline-none"
            />
            <button
              onClick={onClose}
                ...
                {filteredBooks.map((book, index) => (
                  <button
                    ...
                    className={`flex w-full items-center gap-4 rounded-xl px-4 py-3 text-left transition-colors ${
                      index === selectedIndex
                        ? 'bg-sue/10 text-foreground'
                        : 'text-muted-foreground hover:bg-elevated'
                    }`}
                  >
                    {/* Book Cover Thumbnail */}
                    <div className="relative h-12 w-9 flex-shrink-0 overflow-hidden rounded bg-elevated">
                      {book.cover_url ? (
                        <img
                          src={book.cover_url}
                          alt={book.title}
                    ...
                    {/* Subject */}
                    {book.course_code && (
                      <span className="flex-shrink-0 rounded-md bg-elevated px-2 py-1 text-xs font-medium text-muted-foreground">
                        {book.course_code}
                      </span>
                    )}
                  </button>
          ...
          <div className="border-t border-border px-4 py-2">
            <div className="flex items-center justify-between text-xs text-muted-foreground">
              <span>
                ...
                {filteredBooks.length === 1 ? 'wynik' : 'wyników'}
              </span>
              <div className="flex items-center gap-4">
                <span className="flex items-center gap-1">
                  <kbd className="rounded bg-elevated px-1.5 py-0.5 text-xs">
                    ↑↓
                  </kbd>
                  <span>Nawiguj</span>
                </span>
